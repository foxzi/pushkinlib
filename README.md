...
- 🔊 **Озвучка текста (TTS)** — синтез речи через Silero TTS: выбор голоса, регулировка скорости, непрерывное чтение
...
Ответ — аудиофайл (audio/ogg по умолчанию). Все поля кроме `input` опциональны (по умолчанию: голос xenia, скорость 1.0, формат opus, модель v5_ru).
